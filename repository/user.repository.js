...
exports.getUserByUserId = async (userId) => {
  return await UserInfo.findOne({ entityId: userId }, { _id: 0 }).exec();
};
...
exports.getTrendingTaleByTaleId = async (taleId) => {
  return await TrendingTale.findOne({ taleId: taleId }, { _id: 0 }).exec();
};

exports.getTrendingUserByUserId = async (userId) => {
  return await TrendingUser.findOne({ userId: userId }, { _id: 0 }).exec();
};
